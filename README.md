...
$$
\begin{aligned}
\text{Total Supply Chain Cost} = \; & \text{Production Cost} \\
& + \text{Factory-to-Warehouse Transportation Cost} \\
& + \text{Warehouse-to-Market Transportation Cost} \\
...
$$
BeginningInventory_{w,t} + \sum_{f \in F} Shipment^{FW}_{f,w,t} - \sum_{m \in M} Shipment^{WM}_{w,m,t} = Inventory_{w,t}
$$

For the first planning period, the initial inventory of each distribution center is used as the beginning inventory:

$$
InitialInventory_w + \sum_{f \in F} Shipment^{FW}_{f,w,January} - \sum_{m \in M} Shipment^{WM}_{w,m,January} = Inventory_{w,January}
$$

For subsequent periods:

$$
Inventory_{w,t-1} + \sum_{f \in F} Shipment^{FW}_{f,w,t} - \sum_{m \in M} Shipment^{WM}_{w,m,t} = Inventory_{w,t}
$$
...
$$
\sum_{w \in W} Shipment^{WM}_{w,m,t} = Demand_{m,t}
$$
...
At least 30% of the production from Tehran Factory must be shipped through Qom Distribution Center in each planning period.

$$
Shipment^{FW}_{Tehran,Qom,t} \geq 0.30 \times Production_{Tehran,t}
$$
...
$$
\sum_{f \in F} Shipment^{FW}_{f,Ahvaz,t} \leq 1200
$$
...
$$
Shipment^{WM}_{Mashhad,Mashhad,t} \geq 400
$$
...
$$
Production_{f,t} = \sum_{w \in W} Shipment^{FW}_{f,w,t}
$$
...
$$
Production_{f,t} \leq ProductionCapacity_f \qquad \forall f \in F,\; t \in T
$$
...
$$
Inventory_{w,t} \leq WarehouseCapacity_w \qquad \forall w \in W,\; t \in T
$$
...
$$
InitialInventory_w + \sum_{f \in F} Shipment^{FW}_{f,w,t} = \sum_{m \in M} Shipment^{WM}_{w,m,t} + Inventory_{w,t}
$$
...
$$
\sum_{w \in W} Shipment^{WM}_{w,m,t} = Demand_{m,t} \qquad \forall m \in M,\; t \in T
$$
...
$$
Shipment^{FW}_{Tehran,Qom,t} \geq TehranQomRatio \times Production_{Tehran,t} \qquad \forall t \in T
$$
...
$$
\sum_{f \in F} Shipment^{FW}_{f,Ahvaz,t} \leq AhvazReceivingLimit \qquad \forall t \in T
$$
...
$$
Shipment^{WM}_{Mashhad,Mashhad,t} \geq MashhadMinimumDelivery \qquad \forall t \in T
$$
...
$$
Production_{f,t} = \sum_{w \in W} Shipment^{FW}_{f,w,t} \qquad \forall f \in F,\; t \in T
$$
...
This repository contains the complete supply chain optimization model including:

- Mathematical formulation and documentation
- Python/Pyomo implementation
- Input data files (Excel workbook)
- Optimization results and analysis

## Technologies Used

- **Python**: Programming language
- **Pyomo**: Algebraic modeling language for optimization
- **Gurobi**: State-of-the-art optimization solver
- **Excel**: Data input and management
